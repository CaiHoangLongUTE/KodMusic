        ...
        <div className='flex items-center gap-4'>
          {isLoggedIn ? (
            <>
              ...
              <p className='bg-black py-1 px-3 rounded-2xl text-[15px] cursor-pointer'>Cài đặt ứng dụng</p>
              <div className='relative'>
                ...
                {showDropdown && (
                  <div className='absolute right-0 mt-2 w-48 bg-gray-900 border border-gray-700 rounded-lg shadow-xl z-50'>
                    ...
                    <button onClick={handleLogout} className='w-full text-left px-4 py-2 text-white hover:bg-gray-800 transition'>Đăng xuất</button>
                  </div>
                )}
              </div>
            </>
          ) : (
            <button onClick={() => navigate('/login')} className='bg-white text-black text-[15px] px-6 py-2 rounded-full font-semibold hover:scale-105 transition'>Đăng nhập</button>
          )}
        </div>
      </div>
