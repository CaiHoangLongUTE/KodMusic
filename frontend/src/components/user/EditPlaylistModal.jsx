...
const EditPlaylistModal = ({ playlist, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!userId) {
            toast.error('Vui lòng đăng nhập để chỉnh sửa playlist');
            return;
        }
        ...
        try {
            ...
            toast.error(error.response?.data?.message || 'Lỗi khi cập nhật playlist');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className='fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50'>
            <div className='bg-[#282828] rounded-lg p-6 w-[90%] max-w-md'>
                <h2 className='text-white text-2xl font-bold mb-4'>Chỉnh sửa Playlist</h2>
                <form onSubmit={handleSubmit}>
                    <div className='mb-4'>
                        <label className='text-white block mb-2'>Tên Playlist *</label>
                        <input
                            type='text'
                            value={name}
                    ...
                    <div className='mb-4'>
                        ...
                        {playlist.image && !image && (
                            <p className='text-gray-400 text-sm mt-1'>Ảnh hiện tại sẽ được giữ nguyên nếu không chọn ảnh mới</p>
                        )}
                    </div>
